...
export default class Server<
  Socket extends AbstractSocket = any,
  RequestMessage = any,
  ServerImplementation extends AbstractServer<Socket, RequestMessage> = any,
  S = void,
> extends Side<
  Socket,
  { id: number; socket: Socket; session: S },
  CBIndexType
> {
  ...
  public call<Req, Resp, M extends Method<Req, Resp>>(
    socket: Socket,
    method: M,
  ) {
  ...
  broadcastEvent<E extends Event<any>>(event: E) {
    Array.from(this.clients).map(([_, x]) => event.withs(this, x));
  }
